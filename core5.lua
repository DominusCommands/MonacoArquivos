local eventCache = { state = false, updatedAt = 0 }

function isPlayingEvent()
	local timeNow = GetGameTimer()

	if timeNow > (eventCache.updatedAt + 1000) then
		eventCache.state = LocalPlayer.state.inPvp or LocalPlayer.state.inLateGame
		eventCache.updatedAt = timeNow
	end
	
	return eventCache.state
end
...
CreateThread(function()
	...
	while true do
		...
		if LocalPlayer["state"]["Active"] and not LocalPlayer.state.inArena and not LocalPlayer.state.inRoyale and not isPlayingEvent() then
			local Ped = PlayerPedId()
