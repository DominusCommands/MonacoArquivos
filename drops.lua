...
function Creative.Drops(Item,Slot,Amount)
	local source = source
	local Quantity = parseInt(Amount,true)
	local Passport = vRP.Passport(source)

	local IsEligible = Passport 
		and not Active[Passport] 
		and Quantity >= 1 
		and not Player(source)["state"]["Handcuff"] 
		and not exports["hud"]:Wanted(Passport) 
		and not vRP.InsideVehicle(source)
	...
	if vRP.CheckDamaged(Item) then
		TriggerClientEvent("inventory:Notify",source,"Aviso","Utilize as lixeiras para descartar itens estragados.","vermelho")
		TriggerClientEvent("inventory:Update",source)
		return false
	end

	if vRP.TakeItem(Passport,Item,Quantity,false,Slot) then
		return exports["inventory"]:Drops(Passport,source,Item,Quantity,true)
	end
	...
	if Amount >= 1 then
		...
		if not Provisory.desc then
			...
			elseif Item == "prescription" and Split[3] then
					Provisory.desc = "Proprietário: <common>"..vRP.FullName(Split[4]).."</common><br>Medicamento: <rare>"..ItemName(Split[3]).."</rare>"
