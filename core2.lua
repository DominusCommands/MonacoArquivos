...
function RegisterCommand(command, callback)
    _RegisterCommand(command, function(...)
        ...
        if not Player(args[1]).state.inPvp then
...
function Creative.Use(Slot,Amount)
	local source = source
	if Player(source).state.inPvp then return end 
	local Slot = tostring(Slot)
	local Amount = parseInt(Amount,true)
	local Passport = vRP.Passport(source)
	if Passport and not Active[Passport] then
...
AddEventHandler("inventory:Parknmeter", function(Entity)
	...
	local Passport = vRP.Passport(source)
	if Passport and not Active[Passport] and ParknmeterNumber then
		if os.time() <= (Parknmeters[ParknmeterNumber] or 0) then
			...
			local RequiredItem = vRP.ConsultItem(Passport,"lockpick",1)
				...
				CreateThread(function()
					...
					if Active[Passport] then
						...
						if not vRP.MaxItens(Passport,Result["Item"],Result["Valuation"]) and vRP.CheckWeight(Passport,Result["Item"],Result["Valuation"]) then
							...
							TriggerClientEvent("Notify",source,"Sucesso","Você sabotou o parquímetro com sucesso e ele irá explodir em breve.","verde",10000)

							SetTimeout(10000,function()
								for _,v in pairs(vRPC.Players(source)) do
									async(function()
										TriggerClientEvent("inventory:Explosion",v,ParknmeterCoords)
									end)
								end
							end)
